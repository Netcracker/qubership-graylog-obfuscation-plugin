...
public class ConfigurationSerializer {
  ...
  public Map<String, Object> serialize(Configuration configuration) {
    ...
    for (SensitiveRegularExpression sensitiveRegularExpression :
      ...
      serializedSensitiveRegularExpression.put(
          "pattern", sensitiveRegularExpression.getPattern().pattern());
      serializedSensitiveRegularExpression.put(
          "importance", sensitiveRegularExpression.getImportance());
  ...
  public void deserialize(Configuration configuration, Map<String, Object> parameters) {
    ...
    try {
      ...
      Optional<List<SensitiveRegularExpression>> sensitiveRegularExpressions =
          extractor
              .getListOfMap("sensitive-regular-expressions", String.class, Object.class)
              .map(this::deserializeSensitiveRegularExpressions);
      Optional<List<RegularExpression>> whiteRegularExpressions =
          extractor
              .getListOfMap("white-regular-expressions", String.class, Object.class)
              .map(this::deserializeWhiteRegularExpressions);
      Optional<List<String>> fieldNames =
          extractor.getList("field-names", String.class).map(this::deserializeFieldNames);
      Optional<List<String>> streamTitles =
          extractor.getList("stream-titles", String.class).map(this::deserializeStreamTitles);
      Optional<TextReplacer> textReplacer =
          extractor.getString("text-replacer").map(this::deserializeTextReplacer);
      ...
      throw new ObfuscationSystemException(
          "The configuration model is invalid. " + "Reason: " + exception.getMessage(), exception);
    }
  }
  ...
  private List<SensitiveRegularExpression> deserializeSensitiveRegularExpressions(
    ...
    for (Map<String, Object> rawSensitiveRegularExpression : rawSensitiveRegularExpressions) {
      ...
      SensitiveRegularExpression regularExpression =
          new SensitiveRegularExpression(id, name, pattern, importance);
      if (sensitiveRegularExpressions.contains(regularExpression)) {
        throw new ObfuscationSystemException(
            "The sensitive regular expressions " + "with id " + id + " already exists");
      } else {
  ...
  private List<RegularExpression> deserializeWhiteRegularExpressions(
      List<Map<String, Object>> rawWhiteRegularExpressions) {
    ...
    for (Map<String, Object> rawWhiteRegularExpression : rawWhiteRegularExpressions) {
      ...
      if (whiteRegularExpressions.contains(regularExpression)) {
        throw new ObfuscationSystemException(
            "The white regular expressions " + "with id " + id + " already exists");
      } else {
  ...
  private int getId(ParameterExtractor extractor) {
    ...
    if (id < 0) {
      throw new ObfuscationSystemException(
          "The id of sensitive regular expressions cannot lower than 0");
    }
  ...
  private String getName(ParameterExtractor extractor) {
    ...
    if (StringUtils.isEmpty(name)) {
      throw new ObfuscationSystemException(
          "The name of sensitive regular expressions cannot be empty");
    }
  ...
  private Pattern getPattern(ParameterExtractor extractor) {
    ...
    if (StringUtils.isEmpty(pattern.pattern())) {
      throw new ObfuscationSystemException(
          "The pattern of sensitive regular expression cannot be empty");
    }
  ...
  private List<String> deserializeStreamTitles(List<String> streamTitles) {
    ...
    for (String streamTitle : streamTitles) {
      ...
      if (!uniqueStreamTitles.add(streamTitle)) {
        throw new ObfuscationSystemException(
            "The stream titles have duplicate title \"" + streamTitle + "\"");
      }
    }
  ...
  private List<String> deserializeFieldNames(List<String> fieldNames) {
    ...
    for (String fieldName : fieldNames) {
      ...
      if (!uniqueFieldNames.add(fieldName)) {
        throw new ObfuscationSystemException(
            "The field names have duplicate name \"" + fieldName + "\"");
      }
    }
  ...
  private TextReplacer deserializeTextReplacer(String textReplacerName) {
    ...
    if (optionalTextReplacer.isPresent()) {
      ...
      throw new ObfuscationSystemException(
          "The text replacer with name " + "\"" + textReplacerName + "\" is not found");
    }
  }
}
