...
export class ObfuscationConfiguration extends React.Component {
  ...
  componentDidUpdate() {
    let expressions = this.getWhiteRegularExpressions()
      .map((regex) => regex.pattern)
      .concat(
        this.getSensitiveRegularExpressions().map((regex) => regex.pattern),
      );
    ...
    if (this.isExpressionsWasUpdated(expressions)) {
      ...
      this.regexTestCompileRequestId = setTimeout(
        () =>
          fetch("POST", this.prefixURL + "/obfuscation/regex/compile/test", {
            expressions,
          }).then((result) => this.updateState({ regexCompileStatus: result })),
        500,
      );
    }
  }
  ...
  reloadConfiguration() {
    ...
    Promise.all([
      this.getWithPrefix("/obfuscation/configuration").then((result) =>
        this.updateState({ ...result }),
      ),
      this.get("/api/streams").then((result) =>
        this.updateState({
          streams: result.streams.map((stream) => stream.title),
        }),
      ),
      this.get("/api/system/fields").then((result) =>
        this.updateState({ fields: result.fields }),
      ),
      this.getWithPrefix("/obfuscation/replacers").then((result) =>
        this.updateState({ "text-replacers": result["text-replacers"] }),
      ),
      this.get("/api/system/messageprocessors/config").then((result) =>
        this.updateState({ messageProcessors: result }),
      ),
    ]).then(() => {
  ...
  installConfiguration() {
    return fetch(
      "POST",
      this.prefixURL + "/obfuscation/configuration",
  ...
  saveConfig() {
    return () =>
      this.installConfiguration()
        ...
        .catch((error) =>
          this.updateState({ error: error.additional?.body || error.message }),
        );
  }

  resetConfig() {
    return () => {
      fetch("PUT", this.prefixURL + "/obfuscation/configuration/reset").then(
        this.reloadConfiguration,
      );
    };
  }
    ...
    return () => {
      this.updateState({
        "is-obfuscation-enabled":
          this.refs["is-obfuscation-enabled"].getChecked(),
      });
    };
  }
  ...
  setTextReplacer() {
    return () =>
      this.updateState({
        "text-replacer": this.refs["text-replacer"].getValue(),
      });
  }
  ...
  setStreamTitles() {
    return (streamTitles) =>
      this.updateState({ "stream-titles": streamTitles });
  }

  setSensitiveRegularExpressions() {
    return (regularExpressions) =>
      this.updateState({ "sensitive-regular-expressions": regularExpressions });
  }

  setWhiteRegularExpressions() {
    return (regularExpressions) =>
      this.updateState({ "white-regular-expressions": regularExpressions });
  }
  ...
  checkMessageProcessors() {
    ...
    for (let requiredProcessor of this.requiredProcessorNames) {
      ...
      if (
        processorsOrder.indexOf(requiredProcessor) > obfuscationProcessorIndex
      ) {
        return {
          valid: false,
  ...
  getDisabledProcessors() {
    ...
    return this.state.messageProcessors["disabled_processors"].map(
      (processor) => processorClassNameMap[processor],
    );
  }

  getProcessorsOrder() {
    return this.state.messageProcessors["processor_order"].map(
  ...
  isDuplicateStreamTitle(streamTitle) {
    return (
      this.getStreamTitles().filter((title) => title === streamTitle).length > 1
    );
  }
  ...
  isDuplicateId(expectedId, values) {
    return (
      values.map((element) => element.id).filter((id) => id === expectedId)
        .length > 1
    );
  }
  ...
  renderTextReplacers() {
    ...
    for (let textReplacer of this.state["text-replacers"]) {
      ...
      if (replacerName === this.getTextReplacer()) {
        element = (
          <option name={replacerName} selected={true}>
            {body}
          </option>
        );
      } else {
  ...
  render() {
    return (
      <div>
        ...
        <p>Base Configuration for Obfuscation Processor plugin for Graylog</p>
        ...
        <IfPermitted permissions="clusterconfigentry:edit">
          <Button bsStyle="info" bsSize="xs" onClick={this.openModal()}>
            Configure
          </Button>
        </IfPermitted>
            ...
            {!this.isReady() ? (
              ...
              <fieldset>
                ...
                <Input
                  ...
                  help={
                    <span>The pattern which will used for obfuscated data</span>
                  }
                >
                  {this.renderTextReplacers()}
                </Input>
                <OneFieldObjectsTable
                  tableName="Field Names"
                  ...
                  help={
                    <span>
                      The fields will be obfuscated in the log message. The
                      obfuscation will be applied only to text fields
                    </span>
                  }
                />
                <OneFieldObjectsTable
                  tableName="Stream Titles"
                  ...
                  help={
                    <span>The streams whose messages will be obfuscated</span>
                  }
                />
                <RegularExpressionsObjectsTable
                  ...
                  help={
                    <span>
                      The regular expressions which will used for search of
                      sensitive data
                    </span>
                  }
                />
                <RegularExpressionsObjectsTable
                  ...
                  help={
                    <span>
                      The regular expressions which will used for skip white
                      data
                    </span>
                  }
                />
