...
export const Button = ({children, ...props}) => <button type="button" {...props}>{children}</button>;
export const HelpBlock = ({children}) => <div>{children}</div>;
export const Icon = ({name, onClick}) => <button aria-label={name} type="button" onClick={onClick}/>;
